            ...
            if (obss.length > 0) {
                ...
                if(this.labChartLabels.length > 0){
	                ...
	                if( ('hiNormal' in this.labMetaDataDict[this.labConceptDisplay.uuid]) && this.labMetaDataDict[this.labConceptDisplay.uuid].hiNormal != null){
	                	this.labChartOptions.annotation.annotations.push({type:'line',mode:'horizontal',scaleID:'y-axis-0',borderDash: [2, 2],borderDashOffset: 5,value:this.labMetaDataDict[this.labConceptDisplay.uuid].hiNormal,borderColor:'red',borderWidth:2,'label':{position:'top',backgroundColor: 'rgba(255,0,0,1)',fontStyle: 'bold',enabled: true, content: this.labMetaDataDict[this.labConceptDisplay.uuid].shortName+" - high:"+this.labMetaDataDict[this.labConceptDisplay.uuid].hiNormal} });
	            	}
			    	if( ('lowNormal' in this.labMetaDataDict[this.labConceptDisplay.uuid]) && this.labMetaDataDict[this.labConceptDisplay.uuid].lowNormal != null){
			    		this.labChartOptions.annotation.annotations.push({type:'line',mode:'horizontal',scaleID:'y-axis-0',borderDash: [2, 2],borderDashOffset: 5,value:this.labMetaDataDict[this.labConceptDisplay.uuid].lowNormal,borderColor:'red',borderWidth:2,'label':{position:'top',backgroundColor: 'rgba(0,255,0,0.3)',fontStyle: 'bold',enabled: true, content: this.labMetaDataDict[this.labConceptDisplay.uuid].shortName+" - low:"+this.labMetaDataDict[this.labConceptDisplay.uuid].lowNormal}});
		        	}
                }
	    	...
	    	this.openmrsRest.list('obs',{ patient: this.config.patientUuid, v: 'full', limit: this.config.maxResults, concept: this.vitalsSelected[i] }).then((resp) => {
	            ...
	            if (obss.length > 0) {
			    	...
			    	if( ('lowNormal' in this.vitalsMetaDataDict[this.vitalConceptDisplay.uuid]) && this.vitalsMetaDataDict[this.vitalConceptDisplay.uuid].lowNormal != null){
			    		this.vitalChartOptions.annotation.annotations.push({type:'line',mode:'horizontal',scaleID:(i===0 ? 'y-axis-0' : 'y-axis-1'),borderDash: [2, 2],borderDashOffset: 5,value:this.vitalsMetaDataDict[this.vitalConceptDisplay.uuid].lowNormal,borderColor:'green',borderWidth:2,'label':{position:'top',backgroundColor: 'rgba(0,255,0,0.3)',fontStyle: 'bold',enabled: true, content: this.vitalsMetaDataDict[this.vitalConceptDisplay.uuid].shortName+" - low:"+this.vitalsMetaDataDict[this.vitalConceptDisplay.uuid].lowNormal}});
		        	}
	        }        
	            index++;
    	})
